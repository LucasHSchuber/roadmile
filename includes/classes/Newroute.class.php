...
class Newroute
{
    ...
    // add post
    public function addRoute(string $location, int $kilometers, int $parking, string $date): bool
    {
        ...
        //SQL fråga
        $sql = "INSERT INTO route(location, kilometers, parking, datetime)VALUES('$location', '$kilometers', '$parking', '$date');";
        $this->db->query($sql);
    ...
    {
        $sql = "SELECT * FROM route ORDER BY created DESC;";
        $result = $this->db->query($sql); //lagrar svaret från servern i $result
        return mysqli_fetch_all($result, MYSQLI_ASSOC); // lagrar i associativ array så det blir lättare att skriva ut på sidan
    }
